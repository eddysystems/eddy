...
class TestDen {
  ...
  def arrayLiteral(): Unit = {
    ...
    implicit val env = new Env(Array(Main,f),Map((Main,2),(f,2)), f)
  ...
  def nestedIndexExpMixed(): Unit = {
    val x = new LocalVariableItem("x",ArrayType(ArrayType(CharType)),true)
    implicit val env = new Env(Array(x), Map((x,1)))
    testDen("""x{4,5} = x{2}[5]""", AssignExp(None, IndexExp(IndexExp(x,4),5), IndexExp(IndexExp(x,2),5)))
  }
  ...
  def mapExp(): Unit = {
    ...
    implicit val env = new Env(Array(main,f,x,y), Map((main,2),(f,2),(x,1),(y,1)), f)
    testDen("y = f(x)", Nil)
    notImplemented
  }
  ...
  def genericConsObject(): Unit = {
    ...
    implicit val env = localEnvWithBase().addObjects(Array(A,AC),Map((A,3),(AC,3)))
  ...
  def inheritanceShadowing(): Unit = {
    ...
    implicit val env = new Env(Array(X,Y,Z,Xf,Yf,m,y), Map((y,1),(m,2)))
  ...
  def thisToSuper(): Unit = {
    ...
    val tY = ThisItem(Y)
    implicit val env = new Env(Array(X,Y,Xf,Yf,m,tY), Map((tY,2),(m,2),(Y,2),(Yf,2),(X,3),(Xf,3)))
  ...
  def thisExp(): Unit = {
    ...
    val t = ThisItem(X)
    implicit val env = new Env(Array(X,Xx,x,t), Map((x,1),(X,2),(t,2),(Xx,2)))
  ...
  */
  ...
  @Test def sideEffects() = {
    ...
    implicit val env = new Env(Array(X,cons,f),Map((f,2),(X,3)),f)
  ...
  @Test def shuffleArgs() = {
    ...
    implicit val env = new Env(Array(X,f), Map((X,3),(f,2)), f).addLocalObjects(Array(x,d,s,b))
  ...
  @Test def omittedQualifier() = {
    ...
    implicit val env = new Env(Array(P,Z,Zx,Y,Yx,X,Xx,f,x), Map((Y,3),(X,3),(Xx,2),(f,2),(x,1)),f)
