...
object Semantics {
  ...
  */
  ...
  def denoteExp(e: AExp)(implicit env: Env): Scored[Exp] = e match {
    ...
    case FieldAExp(x,ts,f) => if (ts.isDefined) throw new NotImplementedError("Generics not implemented (FieldExp): " + e) else {
      // first, the ones where x is a type
      val tdens: Scored[Exp] = for {t <- denoteType(x)
                                    fi <- staticFieldScores(t,f)
                                    r <- (e,fi) match {
      ...
      // now, x is a value
      val edens: Scored[Exp] = for {e <- denoteExp(x)
                                    fi <- fieldScores(typeOf(e),f)
                                    r <- (e,fi) match {
                                      case (_, f: EnumConstantItem) => single(EnumConstantExp(f))
                                      case (_, f: StaticFieldItem) => single(StaticFieldExp(f))
                                      case (_, f: FieldItem) => single(FieldExp(e,f))
                                    }}
                                 yield r
      tdens++edens
    }
    ...
    case ApplyAExp(f,xsn,_) => {
      ...
      val adens = denoteArray(f) flatMap { a => typeOf(a) match { case t: ArrayType => index(a,t) } }
