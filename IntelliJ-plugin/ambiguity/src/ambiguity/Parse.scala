...
object Parse {
  ...
  def needsI(a: Action, i: Int): Boolean =
    ("""\$"""+(i+1)+"""(?!\d)""").r.findFirstMatchIn(a).nonEmpty
  ...
  def actionGen(implicit G: Grammar): Code = {
    def action(s: Symbol, p: Prod): Code = p match { case (ss,a) =>
      val vs = ss.zipWithIndex map {case (s,i) => if (G.isSimple(s)) "?" else s"x$i"}
      val ps = (vs,ss).zipped.toList.zipWithIndex collect {case ((v,s),i) if !G.isSimple(s) && needsI(a,i) => s"$v: ${G.ty(s)}"} mkString ", "
      val psr = if (needsR(a)) ps + ", r: Range" else ps
      val act = """\$(\d+|r\b)""".r.replaceAllIn(a, m => {
        val g = m.group(1)
        if (g == "r") "r" else {
          val i = g.toInt-1
      ...
      List(s"def ${actionName(s,p)}($psr): ${G.ty(s)} = $act")
    }
    ...
    val extra = List("import ambiguity.Locations._",
  ...
  def parseGen(G: Grammar, nop: Boolean = false): Code = {
    ...
    lazy val init: Code = "// Convert input and allocate working memory" ::
      method("","Parser",List("_input" -> s"List<Located<${G.token}>>"),List(
        s"final int n = _input.size();",
        s"assert n+1<(1<<$posBits);",
        s"input = new ${G.token}[n];",
        s"ranges = new long[n];",
        s"for (int i=0;i<n;i++) {",
        s"  final Located<${G.token}> tr = _input.head();",
        s"  input[i] = tr.x();",
        s"  ranges[i] = tr.raw();",
        s"  _input = (List<Located<${G.token}>>)_input.tail();",
        s"}",
        s"type = new byte[n];"
      ))
    ...
    def act(n: Symbol, prod: Prod, args: List[String], range: Option[String]): String = {
      assert(!G.isSimple(n))
      val (ss,a) = prod
      val needs = ss.zipWithIndex collect { case (s,i) if !G.isSimple(s) => needsI(a,i) }
      ...
      s"${actionName(n,prod)}(${(as ++ r) mkString ","})"
    }
        ...
        (nons collect {case n if G.nullable(n) => {
          ...
          val make: Code =
            if (G.isSimple(n)) List(s"// $n is simple")
              ...
              s"values.add(${act(n,prod,vs.flatten,None)}); // $comment"
            })}
    ...
    def nonnull(n: Symbol): Code =
      method("private void",n,List("lo" -> "final int","hi" -> "final int", "range" -> "final long"),{
        ...
        def add(prod: Prod, vs: List[String]*): Code = List(
          ...
          else s"values.add(${act(n,prod,vs.toList.flatten,r)});")
    ...
    lazy val parse: Code = "// Parse a token stream" ::
      method(s"public static List<${jty(G.start,Box)}>","parse",List("input" -> s"List<Located<${G.token}>>"),List(
        if (nop) "return null;"
