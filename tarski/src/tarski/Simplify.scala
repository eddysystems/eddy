...
object Simplify {
  def simplify(c: Callable): Callable = c match {
    ...
    case MethodDen(x,f,fr) => MethodDen(x map simplify,f,fr)
    case _:ForwardDen => c
    case NewDen(nr,x,f,fr,ts) => NewDen(nr,x map simplify,f,fr,ts)
  ...
  def simplify(s: Stmt): Stmt = s match {
    ...
    case ReturnStmt(rr,e,env) => ReturnStmt(rr,e map simplify,env)
    ...
    case ForStmt(fr,i,c,sr,u,a,x) => ForStmt(fr,simplify(i),c map simplify,sr,u map simplify,a,simplify(x))
  ...
  def simplify(i: ForInit): ForInit = i match {
    case VarStmt(m,t,tr,vs,env) => VarStmt(m,t,tr,vs map simplify,env)
    case ForExps(xs,sr,env) => ForExps(xs map simplify,sr,env)
  }
  ...
  def simplify(e: Exp): Exp = e match {
    ...
    case FieldExp(x,f,fr) => FieldExp(x map (x => simplifyFieldExp(simplify(x),f)),f,fr)
    ...
    case ApplyExp(f,xs,a,auto) => ApplyExp(simplify(f).asInstanceOf[NormalCallable],xs map simplify,a,auto)
    ...
    case ArrayExp(nr,t,tr,xs,a) => ArrayExp(nr,t,tr,xs map simplify,a)
  ...
  def simplifyFieldExp(e: Exp, f: FieldItem): Exp = e match {
    case CastExp(ty,_,x,true) if isSubitem(x.item,ty.item) && !shadowedInSubtype(f,x.item.asInstanceOf[RefTypeItem]) => x
