/* AST: Abstract syntax generated by parsing
 *
 * eddy's AST corresponds roughly to Java, but is significantly more lenient.
 * For example, there is no separation between types and expressions.  As a
 * general rule, we try to push as much logic downstream to Semantics where
 * more information is available, which improves performance since a single
 * ambiguous input string turns into only a few AST parses which are still
 * ambiguous rather than generating many more less ambiguous AST parses.
 *
 * The AST tracks locations, grouping types, etc. throughout, and some of this
 * information is used by Semantics to adjust probabilities.  For example,
 * AExp has a single unified ApplyAExp which handles both array index and
 * function calls, but ApplyAExp remembers whether the grouping construct was
 * brackets as in f[x] or parens as in f(x).  Semantics allows function calls
 * with brackets and array index with parens, but modifies probabilities
 * accordingly.
 *
 * In addition to data type declarations, this file also includes a few routines
 * called from the grammar for filtering purposes.
 */
